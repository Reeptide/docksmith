...
`COPY` sources cannot leave the build context: the parser rejects absolute paths and `../`, and `collectGlob` re-checks each match through `safepath` to catch a symlink inside the context whose target is not. **The context may be relative** — `build -t x .` is the ordinary invocation — so `safepath` absolutises a root before resolving it; leaving it relative made every legitimate `COPY` fail as `path "payload.txt" escapes .`. `scripts/demo.sh` passes an absolute `$CTX` everywhere else, which is why 59 checks missed it; there is now one that cds into the context and passes `.`. Without both, a build reads the host it happens to run on and stops being reproducible anywhere else.
